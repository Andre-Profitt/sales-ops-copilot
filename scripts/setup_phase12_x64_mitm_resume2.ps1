...
<#
Phase 12 setup - resume v2: tolerate pip's stderr warnings.
#>
...
$ptrBits = (& $pyExe -c "import struct; print(struct.calcsize('P')*8)" 2>&1).Trim()
Log "Python: $((& $pyExe --version 2>&1).ToString().Trim()) (pointer bits=$ptrBits)" Green

# pip install mitmproxy + frida-tools (note: pip writes warnings to stderr, not errors)
Log "Installing mitmproxy + frida-tools (capturing stderr as text, not as PS error)"
$pipArgs = @("-m","pip","install","--user","--upgrade","--no-warn-script-location","mitmproxy","frida-tools")
$pipOut = & $pyExe @pipArgs 2>&1 | Out-String
$exitCode = $LASTEXITCODE
$lines = ($pipOut -split "`n" | Where-Object { $_.Trim() }) | Select-Object -Last 8
$lines | ForEach-Object { Log "  pip: $_" }
if ($exitCode -ne 0) {
    Log "pip exited $exitCode — full tail follows" Red
    ($pipOut -split "`n" | Select-Object -Last 30) | ForEach-Object { Write-Host "  $_" }
    exit $exitCode
}

# Verify mitmproxy importable
$mitmVer = (& $pyExe -c "import mitmproxy; print(mitmproxy.__version__)" 2>&1).ToString().Trim()
...
$fridaVer = (& $pyExe -c "import frida; print(frida.__version__)" 2>&1).ToString().Trim()
...
# Locate mitmdump.exe
$userBase = (& $pyExe -c "import site; print(site.USER_BASE)" 2>&1).ToString().Trim()
$candidates = @(
...
# Final state report (single JSON object on stdout, easy to grep)
$state = [ordered]@{
