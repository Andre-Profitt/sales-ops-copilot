#!/usr/bin/env python3
"""Run the PowerPoint Shape.Tag enumeration probe on the Windows VM."""
...
PROBE_PS1 = ROOT / "scripts" / "probe_thinkcell_shape_tags.ps1"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a .pptx (Mac local). Defaults to assets/LAND_thinkcell_seed.pptx.",
    )
    ...
    if output is None:
        ...
        output = (
            ...
            / "shape_tags"
            / stamp
            / "thinkcell_shape_tags_probe.json"
        )
    ...
    deck_arg = ""
    if args.deck:
        deck_arg = f' -DeckPath "{_to_unc(args.deck.expanduser().resolve())}"'

    ps = "\n".join(
        [
            '$ErrorActionPreference = "Stop"',
            '$ProgressPreference = "SilentlyContinue"',
            f'& "{_to_unc(PROBE_PS1)}" -OutputPath "{_to_unc(output)}"' + deck_arg,
        ]
    )
