<#
SSH-triggered wrapper for the MSAA / oleacc probe.
Mirrors _trigger_invokables_probe.ps1: ensures PowerPoint is up under Session 1
via schtasks, then fires the probe via a second scheduled task so the COM /
MSAA calls execute in the interactive desktop session.
#>
$ErrorActionPreference = "Continue"

Copy-Item -LiteralPath "\\Mac\Home\code\apps\sales-ops-copilot\scripts\probe_thinkcell_ribbon_msaa.ps1" `
    -Destination "$env:USERPROFILE\tc_msaa.ps1" -Force
Remove-Item "$env:USERPROFILE\tc_msaa.json" -Force -ErrorAction SilentlyContinue

# Ensure PowerPoint is running with a deck before launching the MSAA walk task.
...
'@ | Set-Content -LiteralPath $launchPath -Force
    ...
    if (-not $pp) { Write-Host "WARNING: PowerPoint didn't come up within 30s" }
}

$tn = "tc_msaa_$(Get-Date -Format yyyyMMddHHmmss)"
$cmd = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$env:USERPROFILE\tc_msaa.ps1`""
