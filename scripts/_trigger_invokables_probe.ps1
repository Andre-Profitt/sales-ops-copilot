<#
SSH-triggered wrapper for the invokables UIA probe.
#>
$ErrorActionPreference = "Continue"

Copy-Item -LiteralPath "\\Mac\Home\code\apps\sales-ops-copilot\scripts\probe_thinkcell_ribbon_invokables.ps1" `
    -Destination "$env:USERPROFILE\tc_inv.ps1" -Force
Remove-Item "$env:USERPROFILE\tc_invokables.json" -Force -ErrorAction SilentlyContinue

# Ensure PowerPoint is running with a deck before launching the UIA walk task.
...
$tn = "tc_inv_$(Get-Date -Format yyyyMMddHHmmss)"
$cmd = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$env:USERPROFILE\tc_inv.ps1`""
