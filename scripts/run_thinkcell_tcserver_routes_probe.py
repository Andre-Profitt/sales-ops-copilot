#!/usr/bin/env python3
"""Run tcserver.exe controlled-start route probe on Windows VM."""
...
PROBE = ROOT / "scripts" / "probe_thinkcell_tcserver_routes.ps1"
...
def main():
    ...
    ps = "\n".join(
        [
            ...
            f'& "{_to_unc(PROBE)}" -OutputPath "{_to_unc(out)}" -Port {a.port}',
        ]
    )
