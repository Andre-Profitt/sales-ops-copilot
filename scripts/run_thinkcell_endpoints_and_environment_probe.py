#!/usr/bin/env python3
"""Run the combined endpoints + environment probe on the Windows VM."""
...
PROBE_PS1 = ROOT / "scripts" / "probe_thinkcell_endpoints_and_environment.ps1"
...
def main() -> int:
    ...
    parser.add_argument("--max-string-hits", type=int, default=800)
    ...
    if output is None:
        ...
        output = (
            ...
            / "state"
            / "thinkcell_bridge"
            / "endpoints_and_environment"
            / stamp
            / "thinkcell_endpoints_and_environment_probe.json"
        )
    ...
    ps = "\n".join(
        [
            ...
            f'& "{_to_unc(PROBE_PS1)}" -OutputPath "{_to_unc(output)}" -MaxStringHits {args.max_string_hits}',
        ]
    )
