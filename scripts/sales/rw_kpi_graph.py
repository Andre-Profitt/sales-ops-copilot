...
_KPIS = (
    ...
    SalesKPI(
        ...
        soql_hint="OpportunityHistory + custom Approval object; flag opps in Stage 3+ without approval",
    ...
    SalesKPI(
        ...
        soql_hint="DATEDIFF(CloseDate, Approval_Date__c) WHERE IsWon=true",
    ),
    SalesKPI(
    ...
    SalesKPI(
        ...
        soql_hint="SUM of active subscription value as of report date",
    ),
    SalesKPI(
    ...
    SalesKPI(
        ...
        soql_hint="WHERE Type='Expand' AND Account.Acquisition_Source__c='Axioma' (or similar)",
        caveats=("Real gap — needs Account.Acquisition_Source__c field confirmation.",),
    ),
    SalesKPI(
    ...
    SalesKPI(
        ...
        soql_hint="SUM(active subscription ARR) WHERE Health_Score__c='At Risk' / total active",
    ),
    ...
    SalesKPI(
        ...
        soql_hint="SUM(PS_ARR__c) / SUM(License_ARR__c) for Won opps in period",
        caveats=("Real gap — 0 SF reports match. May need new field or new build.",),
    ),
    SalesKPI(
        ...
        soql_hint="SUM(ARR) WHERE Deployment_Type__c='SaaS' over time, YoY %",
        caveats=("1 SF report matches; may need consolidation or new dedicated report.",),
    ),
)
