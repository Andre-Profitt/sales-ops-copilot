...
def _link_text(slide: Any, left: float, top: float, width: float, height: float, text: str, url: str) -> Any:
...
def _metric_tile(
    ...
    *,
    ...
    stripe = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(left), Inches(top), Inches(0.055), Inches(0.88))
    stripe.fill.solid()
...
def _status_pill(slide: Any, left: float, top: float, text: str, color: RGBColor) -> None:
    ...
    pill = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(left), Inches(top), Inches(width), Inches(0.18))
    pill.fill.solid()
    pill.fill.fore_color.rgb = color
    pill.fill.transparency = 15
    pill.line.fill.background()
    _text(slide, left + 0.07, top + 0.035, width - 0.14, 0.10, text, 5.6, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
def _value_bar(
    ...
    *,
    ...
    track = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(left + 1.55), Inches(top + 0.035), Inches(width - 2.25), Inches(0.10))
    track.fill.solid()
    track.fill.fore_color.rgb = RGBColor(0xE8, 0xEC, 0xF3)
    track.line.fill.background()
    filled = max(0.02, min(width - 2.25, (width - 2.25) * (value / max(max_value, 1.0))))
    bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(left + 1.55), Inches(top + 0.035), Inches(filled), Inches(0.10))
    bar.fill.solid()
    bar.fill.fore_color.rgb = color
    bar.line.fill.background()
    _text(slide, left + width - 0.56, top - 0.005, 0.56, 0.16, value_label or _meur(value), 6.1, bold=True, color=NAVY, align=PP_ALIGN.RIGHT)
...
def _draw_map(slide: Any, deals: list[Deal], left: float, top: float, width: float, height: float) -> None:
    _soft_panel(slide, left, top, width, height, fill=WHITE)
    _text(slide, left + 0.18, top + 0.15, width - 0.36, 0.20, "Q2 close inspection map", 10.4, bold=True)
    _text(slide, left + 0.18, top + 0.40, width - 0.36, 0.21, "X = days to close; Y = unweighted ARR; color = readiness risk.", 6.4, color=MUTED)
    plot_l = left + 0.50
    plot_t = top + 0.88
    plot_w = width - 0.80
    plot_h = height - 1.35
    ...
    decision_window = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(plot_l), Inches(plot_t), Inches(window_w), Inches(plot_h))
    ...
    _line(slide, plot_l, high_value_y, plot_l + plot_w, high_value_y, RGBColor(0xD5, 0xAF, 0x52), 0.55)
    _text(slide, plot_l + plot_w - 1.08, high_value_y - 0.15, 1.0, 0.12, "EUR 1M line", 5.5, color=AMBER, align=PP_ALIGN.RIGHT)
    ...
    for frac in [0.25, 0.50, 0.75, 1.00]:
        ...
        _text(slide, plot_l - 0.44, y - 0.06, 0.34, 0.12, f"{value:.1f}", 5.8, color=MUTED, align=PP_ALIGN.RIGHT)
    ...
    for deal in deals:
        ...
        bubble = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(x - diameter / 2), Inches(y - diameter / 2), Inches(diameter), Inches(diameter))
        bubble.fill.solid()
        ...
        _text(slide, x + 0.05, y - 0.16 + repeat * 0.12, 0.85, 0.13, label, 5.7, bold=True, color=NAVY)
...
def _draw_forecast_composition(
    ...
    _soft_panel(slide, left, top, width, height, fill=WHITE)
    _text(slide, left + 0.16, top + 0.14, width - 0.32, 0.18, "Forecast composition", 9.2, bold=True)
    _text(slide, left + 0.16, top + 0.38, width - 0.32, 0.16, "Current-quarter Land+Expand ARR only.", 6.1, color=MUTED)
    x = left + 0.18
    bar_y = top + 0.74
    bar_w = width - 0.36
    if total <= 0:
        track = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(x), Inches(bar_y), Inches(bar_w), Inches(0.22))
        track.fill.solid()
        track.fill.fore_color.rgb = RGBColor(0xE8, 0xEC, 0xF3)
        track.line.fill.background()
        _text(slide, x + 0.10, bar_y + 0.055, bar_w - 0.20, 0.08, "No current-quarter forecast ARR", 5.5, color=MUTED, align=PP_ALIGN.CENTER)
    else:
        for label, value, color in values:
            if value <= 0:
                continue
            seg_w = max(0.10, bar_w * value / total)
            segment = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(x), Inches(bar_y), Inches(seg_w), Inches(0.22))
            segment.fill.solid()
    ...
    for idx, (label, value, color) in enumerate(values):
        ...
        dot = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(lx), Inches(top + 1.13), Inches(0.08), Inches(0.08))
        dot.fill.solid()
        dot.fill.fore_color.rgb = color
        dot.line.fill.background()
        _text(slide, lx + 0.11, top + 1.10, 0.78, 0.12, label, 5.8, color=MUTED)
        _text(slide, lx + 0.92, top + 1.10, 0.48, 0.12, _meur(value), 5.8, bold=True, color=NAVY, align=PP_ALIGN.RIGHT)


def _draw_renewal_timeline(slide: Any, renewals: list[dict[str, Any]], left: float, top: float, width: float, height: float) -> None:
    ...
    if len({date for _, date in dated}) < 2:
        _soft_panel(slide, left, top, width, height, fill=WHITE)
        _text(slide, left + 0.16, top + 0.16, width - 0.32, 0.18, "Renewal timing proof", 9.2, bold=True)
        _text(slide, left + 0.16, top + 0.43, width - 0.32, 0.18, "Timeline suppressed: renewal dates are missing or not distinct enough for a timing chart.", 6.2, color=MUTED)
        return
    ...
    _soft_panel(slide, left, top, width, height, fill=WHITE)
    _text(slide, left + 0.16, top + 0.12, width - 0.32, 0.18, "Renewal timing proof", 9.2, bold=True)
    _text(slide, left + 0.16, top + 0.34, width - 0.32, 0.16, "Dots scale by ACV; Renewal ACV stays separate from ARR.", 5.9, color=MUTED)
    axis_l = left + 0.54
    ...
    for date in [min_date, max_date]:
        ...
        _text(slide, x - 0.35, axis_y + 0.16, 0.70, 0.12, f"{date:%b} {date.day}", 5.6, color=MUTED, align=PP_ALIGN.CENTER)
    for row, date in dated[:6]:
        ...
        dot = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(x - diameter / 2), Inches(axis_y - diameter / 2), Inches(diameter), Inches(diameter))
        dot.fill.solid()
        dot.fill.fore_color.rgb = color
        dot.line.color.rgb = WHITE
        dot.line.width = Pt(0.8)
        label = _short_account(str(row.get("account") or ""), str(row.get("name") or ""))
        _text(slide, x - 0.52, axis_y - 0.50, 1.04, 0.12, label, 5.4, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
...
def _build_exec_summary_slide(prs: Presentation, period: str, slug: str, trends: dict[str, Any], original: dict[str, Any]) -> None:
    slide = prs.slides[1]
    ...
    _text(slide, 0.58, 0.95, 10.9, 0.23, "Sales Director operating review. Focus the meeting on close evidence, governance gaps, and renewal ACV basis.", 8.3, color=MUTED)
    _line(slide, 0.58, 1.26, 11.75, 1.26, RULE, 0.8)

    _metric_tile(slide, 0.58, 1.47, 2.58, "Q2 closeable L+E ARR", _meur(_kpi(trends, "total_pipeline_arr")), "Unweighted ARR.", BLUE, fill=PANEL_BLUE)
    _metric_tile(slide, 3.34, 1.47, 2.58, "Commit proof", _meur(commit_arr), "Needs dated evidence.", TEAL, fill=PANEL_TEAL)
    _metric_tile(slide, 6.10, 1.47, 2.58, "Pipeline upside", _meur(pipeline_arr), "Conversion risk.", PURPLE)
    _metric_tile(slide, 8.86, 1.47, 2.58, "Renewal ACV", _meur(renewal_acv), "ACV separate.", AMBER, fill=PANEL_AMBER)
    ...
    _text(slide, 0.78, 3.18, 4.72, 0.26, "This deck is an operating review, so the best visuals are proof objects tied to decisions, not decorative chart variety.", 6.7, color=MUTED)
    max_bar = max(approval_arr, stale_arr, _kpi(trends, "total_pipeline_arr"), 1.0)
    _value_bar(slide, 0.78, 3.70, 4.72, "Closeable ARR", _kpi(trends, "total_pipeline_arr"), max_bar, BLUE)
    ...
    _text(slide, 0.78, 4.86, 4.76, 0.16, f"{high_watch} named Q2 deals need high/watch treatment before forecast sign-off.", 6.4, bold=True, color=NAVY)
    ...
    for idx, (num, title, body, color) in enumerate(steps):
        y = 3.28 + idx * 0.42
        marker = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(6.45), Inches(y), Inches(0.22), Inches(0.22))
        marker.fill.solid()
        marker.fill.fore_color.rgb = color
        marker.line.fill.background()
        _text(slide, 6.505, y + 0.035, 0.11, 0.08, num, 5.4, bold=True, color=WHITE, align=PP_ALIGN.CENTER)
...
def _build_q1_slide(prs: Presentation, trends: dict[str, Any], original: dict[str, Any]) -> None:
    ...
    q1_opened = float(original.get("q1_promised_opened_arr_eur") or original.get("open_land_arr_eur_sidecar") or 0)
    ...
    _text(slide, 0.58, 1.02, 10.9, 0.24, "Separate historical Land accountability from the current May Q2 ARR call.", 8.4, color=MUTED)
    _line(slide, 0.58, 1.36, 11.75, 1.36, RULE, 0.8)
    _tile(slide, 0.58, 1.62, 2.55, "Q1 opened baseline", _meur(q1_opened), "Original/current baseline.", BLUE)
    _tile(slide, 3.40, 1.62, 2.55, "Q1 Land won", f"{q1_won} / {_meur(q1_won_arr)}", "Land ARR only.", TEAL, value_size=14.0)
    _tile(slide, 6.22, 1.62, 2.55, "Q1 lost", f"{q1_lost} / {_meur(q1_lost_arr)}", "Loss accountability.", RED, value_size=14.0)
    _tile(slide, 9.04, 1.62, 2.55, "Q1 slips", f"{q1_slips} / {_meur(q1_slip_arr)}", "Use as coaching lens.", AMBER, value_size=14.0)
    if is_apac:
        rows = [
            ("Current Q2 closeable", _meur(_kpi(trends, "total_pipeline_arr")), "Land+Expand unweighted ARR; Renewal ACV excluded."),
            ("QTD L+E result", "Won/Lost shown later", "Keep Land+Expand result separate from Renewal ACV."),
            ("Since-last-review delta", "open Land 6 -> 12; approved-2026 deals moved 1 -> 2", "Use original APAC pack as the accountability baseline."),
            ("Original Q2 baseline", "6 deals / EUR 5.0M; zero recent activity", "Every Q2 deal needs dated next step and owner evidence."),
        ]
    else:
        rows = [
            ("Current Q2 closeable", _meur(_kpi(trends, "total_pipeline_arr")), "Land+Expand unweighted ARR; Renewal ACV excluded."),
            ("QTD L+E result", "Won/Lost shown later", "Keep Land+Expand result separate from Renewal ACV."),
            ("Original/current Q2 baseline", f"{int(float(original.get('q2_original_deals') or 0))} deals / {_meur(float(original.get('q2_original_arr_eur') or 0))}", "Use as directional context when available."),
            ("May review use", "Confirm, reset, or remove", "Every Q2 deal needs dated next step and owner evidence."),
        ]
    _simple_table(slide, rows, ["Lens", "Current fact", "May implication"], 0.58, 3.05, 11.2, 2.22, [2.15, 2.60, 6.45])
    _source_strip(slide, "Source: connected_factory.xlsx::Raw_Original_Intel + trends.json. ARR = Land+Expand; Renewal ACV separate.")
...
def _build_forecast_slide(prs: Presentation, period: str, slug: str, trends: dict[str, Any], deals: list[Deal], selected: list[Deal], original: dict[str, Any]) -> None:
    slide = prs.slides[3]
    _remove_all_shapes(slide)
    ...
    forecast = _forecast_summary(period, slug)
    commit_bucket = forecast.get("Commit", {"count": float(len(commit)), "arr_eur": sum(d.arr_eur for d in commit)})
    pipeline_bucket = forecast.get("Pipeline", {"count": float(len(pipeline)), "arr_eur": sum(d.arr_eur for d in pipeline)})
    ...
    _line(slide, 0.58, 1.36, 11.75, 1.36, RULE, 0.8)
    _tile(slide, 0.58, 1.58, 2.62, "Closeable L+E ARR", _meur(_kpi(trends, "total_pipeline_arr")), "Unweighted current-quarter ARR.", BLUE)
    _tile(slide, 3.48, 1.58, 2.62, "Commit ARR", _meur(commit_bucket["arr_eur"]), f"{int(commit_bucket['count'])} opps; {sum(1 for d in selected if d.forecast == 'Commit')} material named.", TEAL)
    _tile(slide, 6.38, 1.58, 2.62, "Pipeline ARR", _meur(pipeline_bucket["arr_eur"]), f"{int(pipeline_bucket['count'])} opps; {sum(1 for d in selected if d.forecast == 'Pipeline')} material named.", PURPLE)
    _tile(slide, 9.28, 1.58, 2.62, "Best Case ARR", _meur(best_case_bucket["arr_eur"]), "No material cushion." if best_case_bucket["arr_eur"] < 100_000 else f"{int(best_case_bucket['count'])} opps.", RED)
    ...
    rows = [
        ("Commit proof", _short(", ".join(_short_account(d.account, d.opportunity) for d in commit[:5]) or "No material Commit rows", 58), "Attach dated next step / decision evidence."),
        ("Pipeline upside", _short(", ".join(_short_account(d.account, d.opportunity) for d in pipeline[:5]) or "No material Pipeline rows", 58), "Confirm conversion evidence or reset forecast category."),
        ("Original forecast spine", original_forecast_fact, "Keep weighted/unweighted labeled."),
        ("Risk to call", "No Best Case cushion" if best_case_bucket["arr_eur"] < 100_000 else "Best Case exists but needs proof", "Use deal inspection, not forecast optimism."),
    ]
    _simple_table(slide, rows, ["Issue", "Named evidence", "May action"], 0.58, 3.06, 6.62, 2.18, [1.55, 2.42, 2.65], font_size=6.15)
    _source_strip(slide, "Source: trends.json::kpis + q2_deal_readiness; original PI context from Raw_Original_Intel.", action_label="Open no-activity CFQ report", action_url=REPORT_LINKS["activity_drought"])
...
def _build_q2_slide(prs: Presentation, deals: list[Deal], selected: list[Deal]) -> None:
    ...
    _text(slide, 0.58, 0.94, 10.9, 0.25, "Salesforce snapshot as of 2026-04-30. Land+Expand unweighted ARR only; Renewal ACV remains separate.", 8.4, color=MUTED)
    _line(slide, 0.58, 1.30, 11.75, 1.30, RULE, 0.8)
    commit_names = ", ".join(_short_account(d.account, d.opportunity) for d in selected if d.forecast == "Commit") or "named Commit rows"
    pipe_names = ", ".join(_short_account(d.account, d.opportunity) for d in selected if d.forecast == "Pipeline") or "named Pipeline rows"
    _text(slide, 0.58, 1.49, 3.0, 0.22, "Operating read", 10.2, bold=True)
    _text(slide, 0.58, 1.78, 4.45, 0.86, f"Q2 close call is not a summary-chart problem: Commit proof depends on {_short(commit_names, 70)}; Pipeline upside depends on {_short(pipe_names, 56)}.", 9.0, color=NAVY)
    _text(slide, 0.58, 2.73, 4.45, 0.72, "Current May review should force dated next steps, activity proof, approval status, and close-date realism for each named row.", 8.1, color=MUTED)
    ...
    _source_strip(slide, "Source: trends.json::q2_deal_readiness + connected_factory.xlsx::Raw_Current_Q2_Readiness.", action_label="Open no-activity CFQ report", action_url=REPORT_LINKS["activity_drought"])


def _deal_table(slide: Any, deals: list[Deal], left: float, top: float, width: float, height: float) -> None:
    rows = len(deals) + 1
    cols = 6
...
def _build_renewal_slide(prs: Presentation, trends: dict[str, Any], original: dict[str, Any]) -> None:
    ...
    q2_total = sum(float(row.get("acv_eur") or 0) for row in all_renewals if str(row.get("close_date") or "") <= "2026-06-30")
    original_total = float(original.get("fy26_renewals_acv_eur_original") or original.get("q2_renewals_acv_eur") or 0)
    ...
    _line(slide, 0.58, 1.36, 11.75, 1.36, RULE, 0.8)
    _tile(slide, 0.58, 1.58, 2.75, "Original/baseline ACV", _meur(original_total), "Prior pack/source context.", TEAL)
    _tile(slide, 3.60, 1.58, 2.75, "Current state workbook", _meur(current_total), f"{len(all_renewals)} FY26 renewals; open ACV.", BLUE)
    _tile(slide, 6.62, 1.58, 2.75, "Current Q2 ACV", _meur(q2_total), "Current-quarter renewal ACV.", AMBER)
    ...
    table_shape = slide.shapes.add_table(len(renewals) + 1, 8, Inches(0.58), Inches(4.30), Inches(11.2), Inches(1.18))
    table = table_shape.table
    ...
    for row_idx, row in enumerate(renewals, 1):
        ...
        for col_idx, value in enumerate(values):
            _set_cell(table.cell(row_idx, col_idx), value, fill=RGBColor(0xF4, 0xF6, 0xFA) if row_idx % 2 == 0 else WHITE, size=5.65 if col_idx in {2, 3} else 5.9, bold=col_idx == 2)
    _text(slide, 0.58, 5.64, 10.8, 0.20, "Monthly review use: keep ACV separate from ARR, name the current-quarter renewal owner, and hold external FY26 ACV quotes until basis differences are reconciled.", 6.6, color=NAVY)
    _source_strip(slide, "Source: trends.json::fy26_renewals + connected_factory.xlsx::Raw_Current_FY26_Renewals. Renewal values are ACV.")
...
def _evidence_names(period: str, slug: str) -> dict[str, str]:
    ...
    approval = [
        r for r in rows
        if is_le(r) and _stage_num(r.get("Stage")) >= 3 and arr(r) >= 500_000 and str(r.get("Approved") or "") != "Yes"
    ]
    zombie = [r for r in rows if is_le(r) and created_old(r, 730) and stale_activity(r, 60)]
    samples: dict[str, str] = {}
    for key, selected in [("activity_drought", activity), ("approval_gap", approval), ("zombie_arr", zombie)]:
        selected.sort(key=arr, reverse=True)
        samples[key] = _short(", ".join(_short_account(str(r.get("Account") or ""), str(r.get("Opportunity") or "")) for r in selected[:6]) or "Open Salesforce report", 72)
    return samples
...
def _build_action_slide(prs: Presentation, period: str, slug: str, trends: dict[str, Any]) -> None:
    ...
    for evidence in coverage_item.get("evidence", []) or []:
        if str(evidence).startswith("sample="):
            ...
            break
    rows = [
        ("Q2 activity drought", samples.get("activity_drought", "Open report"), action_items.get("activity_drought", {}).get("claim", "Q2 activity refresh needed"), "Owners log next step/task before forecast review.", "CFQ no-activity report", REPORT_LINKS["activity_drought"]),
        ("Commercial approval", samples.get("approval_gap", "Open report"), action_items.get("approval_gap", {}).get("claim", "Approval gap review needed"), "Submit/confirm Stage 3 approval gate.", "Pending approval report", REPORT_LINKS["approval_gap"]),
        ("Zombie pipeline", samples.get("zombie_arr", "Open report"), action_items.get("zombie_arr", {}).get("claim", "Stale pipeline cleanup needed"), "Rep-by-rep close/disqualify review by EOM.", "Zombie report", REPORT_LINKS["zombie_arr"]),
        ("Tier-1 coverage", _short(coverage_sample or "Open report", 72), coverage_item.get("claim", "Coverage-gap review needed"), "Assign opener ownership and coverage plan.", "Coverage gap report", REPORT_LINKS["coverage_gap"]),
    ]
    ...
    for row_idx, row in enumerate(rows, 1):
        for col_idx, value in enumerate(row[:5]):
            _set_cell(table.cell(row_idx, col_idx), value, fill=RGBColor(0xF4, 0xF6, 0xFA) if row_idx % 2 == 0 else WHITE, size=6.0 if col_idx in {1, 2, 3} else 6.15, bold=col_idx == 0, url=row[5] if col_idx == 4 else None)
    _source_strip(slide, "Source: trends.json::action_items + Raw_Pipeline_Open samples + verified Salesforce reports. Counts remain horizon-labeled.", action_label="Open action reports", action_url=REPORT_LINKS["activity_drought"])
...
def enhance_meeting_spine_deck(period: str, slug: str, deck_path: Path) -> dict[str, Any]:
    ...
    if len(prs.slides) < 14:
        raise ValueError(f"deck has {len(prs.slides)} slides; expected meeting spine with at least 14")
