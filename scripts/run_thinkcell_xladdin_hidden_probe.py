#!/usr/bin/env python3
"""Run the Excel-side hidden-surface probe on the Windows VM.

Targets tcXlAddIn and the tcUpdate builder (returned by CreateUpdate)
with the same 3,200-candidate name resolution as the original PowerPoint
...
PROBE_PS1 = ROOT / "scripts" / "probe_thinkcell_xladdin_hidden.ps1"
...
def main() -> int:
    ...
    parser.add_argument("--max-binary-strings", type=int, default=2400)
    parser.add_argument("--max-dispatch-candidates", type=int, default=3200)
    ...
    if output is None:
        ...
        output = (
            ...
            / "xladdin_hidden"
            / stamp
            / "thinkcell_xladdin_hidden_probe.json"
        )
