...
**KPI surface:** 31 KPIs in `scripts/sales/rw_kpi_graph.py`. Spec covers 24 directly; 7 deferred (need data infrastructure we don't have — Asset, ApprovalProcess, lead-source rollups, partner-channel field, deal-size bands).
...
Five tabs at the bottom of the report. Default landing: **What Changed**.
...
| **Growth Mix**    | Strategic-play visibility | "What kind of business are we landing?" |
...
### Stage 3 gate panel (small section between matrix and funnel)

Two cards specific to the S3 governance gate:

- **S3 ACV value** (`stage3_acv_value`) — total open ACV at Stage 3, RAG vs target
- **S3 Commercial Approval compliance** (`stage3_approvals_compliance`) — % of Stage 3+ deals with approval recorded, RAG vs target

KG entries that don't fit the matrix and need a dedicated micro-section.
...
The "what kind of business are we landing" lens. Picks up the Conversion (1.9) and Pricing (1.14) KG areas that don't fit elsewhere.

### Hero strip (top)

Three cards:

- **SaaS ARR YoY growth** (`saas_arr_yoy_growth`, HIGH) — current FY SaaS-motion ARR vs same period last year
- **New customer ARR** (`new_customer_reporting`, HIGH) — net-new logo count + ARR this FQ
- **Synergy deals won** (`synergy_deals_won`, HIGH) — count + ARR of cross-product/segment deals closed-won this FQ

### Conversion mix table (middle)

Land breakdown by conversion subtype. Rows = ILF / ALF / Other; columns = Open ARR · Won ARR FQTD · Pipe count · Win rate.

- `ilf_arr_pipeline` (HIGH, partial) → ILF row
- `alf_arr_pipeline` (HIGH, partial) → ALF row

If subtype categorization isn't decodable from current SF fields, render two rows showing combined Land + flag in Note column. Verify ILF/ALF source field with RW before build.

### Synergy + cross-sell strip (middle)

Two compact tables side-by-side:

- **Synergy pipe vs won:** synergy_deals_pipe (count, ARR) vs synergy_deals_won (count, ARR), conversion %
- **Cross-sell to acquired** (`cross_sell_to_acquired`, HIGH, missing): count + ARR of opps where Account is flagged acquired-via-M&A. If field doesn't exist, render "data not available — needs Account field decision" tile.

### Pricing mix (bottom)

Three cards covering 1.14 Pricing area:

- **PS attach rate** (`ps_arr_attach`, MEDIUM, missing) — % of new ARR with attached Professional Services LineItem. Requires OpportunityLineItem × ProductFamily pull. Render as "data not available" tile until Phase 2 ETL extension.
- **One-off revenue mix** (`one_off_revenues`, MEDIUM, missing) — non-recurring revenue as % of total bookings. Same dependency. Same fallback.
- **SaaS YoY trend** — line chart, 12-month rolling SaaS ARR, derived from existing data (no new dependency).

**Data dependency:** Phase 1 covers SaaS YoY + synergy KPIs (already in semantic model). Phase 2 ETL extension (separate plan) needed for ILF/ALF subtype, cross-sell-to-acquired flag, OpportunityLineItem ProductFamily.

**Honest "not available" treatment:** for KPIs flagged missing, render a tile with the KPI name, target, and a single line "Data source: <SF object> — pending Phase 2 ETL." This is intent-honest and gives RW visibility into what's coming.
...
3. **Create 5 pages** in `rpt_vp_ops_scorecard` via Fabric REST report.json — extend `rw_add_visual.py` with `--build-page <name>` per tab
